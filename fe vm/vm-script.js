...
let connectionMode = ''; // 'local' or 'remote'
...
$(document).ready(function() {
    ...
    $('#localModeBtn').on('click', useLocalBackend);
    ...
    if (savedMode === 'local') {
        // If we have a saved local mode, use it immediately
        setTimeout(() => {
            useLocalBackend();
        }, 500);
...
/**
 * Use local backend (same origin)
 */
function useLocalBackend() {
    connectionMode = 'local';
    localStorage.setItem('connectionMode', 'local');
    
    // Use relative URL which will use same origin as the page
    API_URL = '/api';
    ...
    testConnection()
        .then(() => {
            ...
            getListNotes();
            // Update status
            $('#connectionStatus').text('Connected (Local)').removeClass('text-red-400').addClass('text-green-400');
            // Reset connection attempts
            connectionAttempts = 0;
        })
        .catch(err => {
            console.error('Local connection failed:', err);
            showToast('Local backend connection failed. Please try remote connection instead.', 'error');
...
function connectToBackend() {
    ...
    testConnection()
        .then(() => {
            ...
            getListNotes();
            // Update status
            $('#connectionStatus').text('Connected (Remote)').removeClass('text-red-400').addClass('text-green-400');
            // Reset connection attempts
            connectionAttempts = 0;
        })
        .catch(err => {
